...
let server = "https://lightbox-producer-expect-expert.trycloudflare.com";

console.log("Smart Bundle Discount app new version 8.0");
console.log("sd_metadata_bundles", sdShopMeta);
...
async function getData() {
  ...
  let filterdBundle = filterBundles(sdBundleProductId, collectionId);
  console.log("filterdBundle===>", filterdBundle);
...
function filterBundles(id, collId) {
  let pId = `gid://shopify/Product/${id}`;
  console.log(pId,collId)
  return sdShopMeta.filter((bundle) => {
    const detail = bundle.bundleDetail;
    const type = bundle.type;

    // PRODUCT BUNDLE
    if (type === "productBundle") {
      return  !detail.display.productPages || (detail.display.productPages && detail.display?.productPagesList?.includes(pId));
    }

    // VOLUME BUNDLE
    if (type === "volumeBundle") {
      const dType = detail.discountedProductType;

      if (dType === "specific_product") {
        return !detail.display.productPages || (detail.display.productPages && detail.display?.productPagesList?.includes(pId));
      }

      if (dType === "all_products") {
        return true;
      }

      if (dType === "collection") {
        return !detail.display.productPages || (detail.display.productPages && detail.display?.productPagesList?.some((id) =>
          collId.includes(id)
        ));
      }
    ...
    if (type === "fbt") {
      const mainMatch = detail.mainProducts?.some((p) => p.id == pId);
      const offerMatch = detail.offeredProducts?.some((p) => p.id == pId);
      const allProducts = detail.discountedProductType === "all_products";

      return mainMatch || offerMatch || allProducts;
    }
  ...
  BUNDLE_DATA.forEach((el, bundleIndex) => {
    ...
    if (el?.type == "productBundle") {
      ...
      ADD_TO_CART_BUTTON.addEventListener("click", (e) => {
        ...
        const UNIQUE_ID = generateRandomSixDigitNumber();
        let mergedArrayResult = Object.values(mergedResult).map((item) => ({
          ...item,
          properties: {
            _SD_BUNDLE_ID: el._id+UNIQUE_ID,
          },
        }));
        let formData = {
              ...
              fetch(window.Shopify.routes.root + "cart.js", {
                ...
                .then((response) => {
                  ...
                  if (el.bundleDetail.discountType == "noDiscount") {
                    ...
                    console.log("sd_bundle_data============>", sd_bundle_data);
